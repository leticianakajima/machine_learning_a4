...
if __name__ == "__main__":
    ...
    if question == "2":
        ...
        model = linear_model.logRegL2(lammy=1.0, verbose=0, maxEvals=400)
        model.fit(XBin, yBin)
        ...
        model = linear_model.logRegL1(lammy=1.0, verbose=0, maxEvals=400)
        model.fit(XBin, yBin)
        ...
        model = LogisticRegression('l2',fit_intercept=False)
        model.fit(XBin, yBin)
        print("\nsklearn l2 Training error %.3f" % utils.classification_error(model.predict(XBin), yBin))
        print("sklearn l2 Validation error %.3f" % utils.classification_error(model.predict(XBinValid), yBinValid))
        print("# nonZeros: %d" % (model.coef_ != 0).sum())

        model = LogisticRegression('l1', fit_intercept=False)
        model.fit(XBin, yBin)
        print("\nsklearn l1 Training error %.3f" % utils.classification_error(model.predict(XBin), yBin))
        print("sklearn l1 Validation error %.3f" % utils.classification_error(model.predict(XBinValid), yBinValid))
        print("# nonZeros: %d" % (model.coef_ != 0).sum())
        ...
        model = sklearn.linear_model.LogisticRegression(multi_class = 'multinomial',solver = 'lbfgs', fit_intercept = False, C = 1000)
        model.fit(XMulti, yMulti)
